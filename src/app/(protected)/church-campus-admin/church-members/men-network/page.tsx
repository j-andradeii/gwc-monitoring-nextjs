...
export default function MenNetworkPage() {
  const router = useRouter();
  usePageBreadcrumbs('Men Network');
  ...
  const fetchMembers = useCallback(async () => {
    ...
    const response: MemberListResponseDto | null = await memberService.getMembers({
      ...
      search: 'MALE', // This would be a proper filter in production
    });

    if (response) {
      // Filter by gender on client side (temporary)
      const maleMembers = (response.data || []).filter(
        (m) => m.gender === 'MALE'
      );
      setMembers(maleMembers);
      pagination.setTotalRecords(maleMembers.length);
    }
    setLoading(false);
  ...
  return (
    <div>
      <PageHeader
        title="Men Network"
        subtitle="View all male members"
      />
      ...
      <CardDiv>
        <DataTable
          ...
          emptyMessage="No male members found"
